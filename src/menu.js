 ...
 */
blimpkit
  .directive('bkMenu', function ($window, $timeout, $injector, backdrop, classNames) {
    ...
    return {
      ...
      link: {
        pre: function (scope, _element, attrs, parentCtrls) {
          if (!Object.prototype.hasOwnProperty.call(attrs, 'ariaLabel')) console.error('bk-menu error: You must set the "aria-label" attribute');
          if (!angular.isDefined(scope.show)) scope.show = true;
          if (!angular.isDefined(scope.noBackdrop)) scope.noBackdrop = false;
          if (parentCtrls[0] !== null || parentCtrls[1] !== null) {
            scope.noBackdrop = true;
            scope.noShadow = true;
            scope.closeOnOuterClick = false;
          } else if (scope.noBackdrop) scope.closeOnOuterClick = false;
        ...
        post: function (scope, element) {
          ...
          if (scope.maxHeight) $window.addEventListener('resize', resizeEvent);
          ...
          scope.getMenuClasses = function () {
            ...
            return classNames('fd-menu', {
              'fd-menu--overflow fd-scrollbar': scope.canScroll === true,
              'fd-menu--icons': scope.hasIcons === true,
            });
          };
          scope.getListClasses = () =>
            classNames('fd-menu__list', {
  ...
  .directive('bkMenuItem', (classNames) => ({
    ...
    link: (scope) => {
      ...
      scope.getItemClasses = () =>
        classNames('fd-menu__item', {
          'has-separator': scope.hasSeparator === true,
        });
    },
      ...
      post: function (scope, element) {
        ...
        function pointerupEvent(e) {
          ...
          if (list && list.id === scope.sublistId) {
            ...
            if (e.originalEvent.pointerType !== 'mouse') scope.$apply(scope.show());
          }
        }
        ...
        scope.getClasses = () =>
          classNames('fd-menu__link has-child', {
        ...
        scope.hide = function (event) {
          if (!scope.isDisabled && scope.isExpanded) {
            ...
            } else if (!element[0] === event.currentTarget) {
              // Firefox tooltip fix
              scope.hideSubmenu();
            }
          }
