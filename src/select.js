 ...
 */
blimpkit
  .directive('bkSelect', (uuid, $window, $timeout, ScreenEdgeMargin, classNames) => ({
    restrict: 'EA',
    replace: true,
    transclude: true,
    require: '?ngModel',
    ...
    controller: [
      ...
      function ($scope, $element) {
        ...
        $scope.getClasses = () =>
          classNames('fd-select', {
            'fd-select--compact': $scope.compact === true,
          });

        $scope.getControlClasses = () =>
          classNames('fd-select__control', {
        ...
        $scope.getListClasses = () =>
          classNames('fd-list', 'fd-list--dropdown', {
            'fd-list--has-message': $scope.message,
            'fd-list--compact': $scope.compact === true,
            'fd-list--large-dropdown': $scope.size === 'large',
          });

        $scope.getListMessageClasses = () =>
          classNames('fd-list__message', {
            [`fd-list__message--${$scope.state}`]: $scope.state,
          });

        $scope.getFormMessageClasses = () =>
          classNames('fd-form-message', 'fd-form-message--static', {
        ...
        $scope.getSelectedItem = () => {
          if ($scope.selectedValue === undefined || $scope.selectedValue === null) return null;
    ...
    template: `<div class="fd-popover">
        ...
        <div ng-if="!isReadonly && message && state && messageVisibile" class="fd-popover__body fd-popover__body--no-arrow" aria-hidden="{{ bodyExpanded }}" ng-style="getStyle()">
  ...
  .directive('bkOption', (classNames, uuid) => ({
    ...
    link: (scope, _element, _attrs, selectCtrl) => {
      ...
      scope.getClasses = () =>
        classNames('fd-list__item', {
          'is-selected': scope.isSelected(),
        });

      scope.getTitleClasses = () =>
        classNames('fd-list__title', {
          'fd-list__title--no-wrap': scope.noWrap === true,
        });

      scope.getIconClasses = () =>
        classNames('fd-list__icon', {
