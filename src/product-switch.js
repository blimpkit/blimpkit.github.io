 ...
 */
blimpkit
  .directive('bkProductSwitch', (classNames, $injector, ButtonStates) => {
    ...
    return {
      ...
      link: function (scope) {
        if (!angular.isDefined(scope.btnAriaLabel)) console.error('bk-product-switch error: Must have the "btn-aria-label" attribute');
        scope.getClasses = () =>
          classNames('fd-product-switch__body', {
  ...
  .directive('bkProductSwitchItem', (classNames) => ({
    ...
    link: function (scope) {
      scope.getClasses = () =>
        classNames('fd-product-switch__item', {
