...
blimpkit
    ...
    return {
      getConfig: function () {
        return {
          ...
          link: {
            pre: function (scope) {
              if (scope.instructions) scope.buttonId = `b${uuid.generate()}`;
            },
            post: function (scope, _element, attrs, ctrl) {
              if (!scope.label && (scope.glyph || scope.iconPath) && !Object.prototype.hasOwnProperty.call(attrs, 'ariaLabel')) console.error('bk-button error: Icon-only buttons must have the "aria-label" attribute');
  ...
  .directive('bkButton', function (bkButtonConfig) {
    ...
    buttonConfig['template'] =
      `<button ng-class="getClasses()" ng-attr-aria-disabled="{{ disabledFocusable === true ? true : undefined }}" aria-pressed="{{ toggled === true }}" ng-attr-aria-describedby="{{ instructions ? buttonId : undefined }}">${buttonConfig.innerTemplate}</button>`;
    return buttonConfig;
  })
  .directive('bkSegmentedButton', () => ({
    ...
    link: function (_scope, _element, attrs) {
      if (!Object.prototype.hasOwnProperty.call(attrs, 'ariaLabel')) console.error('bk-segmented-button error: You should provide a description of the group using the "aria-label" attribute');
    },
    ...
    return {
      ...
      controller: [
        ...
        function ($scope, $element, $attrs) {
          ...
          if (!Object.prototype.hasOwnProperty.call($attrs, 'ariaLabel')) console.error('bk-split-button error: You should provide a description of the split button using the "aria-label" attribute');
          $scope.getSplitClasses = () =>
            classNames('fd-button-split', {
              'fd-button--emphasized': $scope.state === ButtonStates.Emphasized,
          ...
          $scope.mainActionClicked = function () {
            if (!$scope.disabledFocusable && $scope.callback) $scope.callback();
          };
