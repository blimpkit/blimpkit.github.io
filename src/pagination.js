...
blimpkit.directive('bkPagination', (uuid, classNames, $injector) => {
  ...
  return {
    ...
    link: (scope) => {
      ...
      scope.getClasses = () =>
        classNames('fd-pagination', {
          'fd-pagination--short': scope.isShortMode(),
        });

      scope.getNumberButtonClasses = (pageNumber) =>
        classNames('fd-button', 'fd-button--transparent', 'fd-pagination__link', {
          'is-active': pageNumber === scope.currentPage,
          'fd-button--compact': scope.compact === true,
        });

      scope.getArrowButtonClassess = () =>
        classNames('fd-button', 'fd-button--transparent', 'fd-pagination__button', {
          'fd-button--compact': scope.compact === true,
        });
      ...
      const itemsWatch = scope.$watch('itemsPerPage', (newVal, oldVal) => {
        if (newVal !== oldVal) {
          if (scope.itemsPerPageChange) scope.itemsPerPageChange({ itemsPerPage: scope.itemsPerPage });
        }
