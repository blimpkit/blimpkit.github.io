...
blimpkit.directive('bkComboboxInput', function (uuid, classNames, $window, $timeout, $injector, ButtonStates) {
  if (
    !$injector.has('bkListDirective') ||
    !$injector.has('bkInputDirective') ||
    !$injector.has('bkTokenizerDirective') ||
    !$injector.has('bkTokenDirective') ||
    !$injector.has('bkButtonDirective') ||
    !$injector.has('bkScrollbarDirective')
  ) {
  ...
  return {
    ...
    link: function (scope, element, _attrs, ngModel) {
      ...
      scope.onSearchKeyDown = (event) => {
        switch (event.key) {
          case 'Backspace':
            if (scope.search.term.length === 0 && scope.selectedValue.length) scope.selectedValue.splice(-1, 1);
            break;
          case 'Enter':
      ...
      scope.removeItemFromSelection = (item) => {
        ...
        if (itemIndex >= 0) scope.selectedValue.splice(itemIndex, 1);
      };

      scope.addItemToSelection = (item) => {
        const itemIndex = scope.selectedValue.indexOf(item.value);
        if (itemIndex === -1) scope.selectedValue.push(item.value);
      };
      ...
      scope.onItemClick = (item) => {
        if (scope.multiSelect) {
          const itemIndex = scope.selectedValue.indexOf(item.value);
          if (itemIndex >= 0) scope.selectedValue.splice(itemIndex, 1);
          else scope.selectedValue.push(item.value);
        } else {
        ...
        if (!scope.multiSelect) scope.closeDropdown();
      };
      ...
      const dropdownWatch = scope.$watchCollection('dropdownItems', (items) => {
        if (items === undefined || items === null) scope.dropdownItems = [];
